...
fileset = Path('graphs').rglob('*.new_indptr')

for file in fileset:
    print(file.stem)
    new_indptr = np.fromfile('graphs/' + file.stem + ".new_indptr", dtype=np.int32)
    new_indices = np.fromfile('graphs/' + file.stem + ".new_indices", dtype=np.int32)
